...
python3 tools/demo_video.py --config configs/smpl/tune.py --image_folder data/posetrack2018/images/val/002374_mpii_test --flow_folder data/posetrack2018/optical_flow/val/002374_mpii_test --output_folder posetrack_baseline/002374_mpii_test --ckpt data/checkpoint.pt
...
def parse_args():
    ...
    parser.add_argument('--flow_folder', help='Path to folder with optical flows')
...
def main():
    ...
    with torch.no_grad():
        ...
        flow_folder_name = args.flow_folder
        output_folder = args.output_folder
        os.makedirs(output_folder, exist_ok=True)
        images = sorted(os.listdir(folder_name))
        flows = sorted(os.listdir(flow_folder_name))
        for (image, flow) in zip(images[1:], flows):
            file_name = osp.join(folder_name, image)
            flow_file = osp.join(flow_folder_name, flow)
            ...
            flow = F.interpolate(flow[None], size=img.shape[-2:], mode='bilinear').squeeze(0)
            padded_img = np.zeros((img.shape[0], img_scale[1], img_scale[0]), dtype=img.dtype)
            padded_flow = torch.zeros((flow.shape[0], img_scale[1], img_scale[0]), dtype=flow.dtype)
            padded_img[:, :img.shape[-2], :img.shape[-1]] = img
            padded_flow[:, :img.shape[-2], :img.shape[-1]] = flow
            img = padded_img
            flow = padded_flow
            ...
            data_batch = dict(
                ...
                flow=DC([to_tensor(flow[None, ...])], stack=True),
